...
export default function CarouselComp({images}){
    const data = images.map(image=>{return{
        "url":image.url,
        "description":image.explanation,
        "title":image.title
    }})
    return (
        <Carousel className="rounded-xl w-[98vw] lg:w-[80vw]" autoplay={true} transition={{ duration: 2 }}
        ...
        >
      {data.map((val,index)=>(
        <div className="w-[98vw] lg:w-[80vw]" key={index}>
        <img
          src={val.url}
          alt={val.title}
